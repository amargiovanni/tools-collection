...
export interface EncodeOptions {
  ...
  readonly mode: 'minimal' | 'all'
}

/**
 * Map of characters to their named HTML entities (encode direction).
 * Covers the most common named entities.
 */
...
/** Minimal encoding: only the five XML-special characters */
const minimalChars = new Set(['&', '<', '>', '"', "'"])

/**
 * Reverse map: named entity string -> character.
 * Built once from charToEntity.
 */
const entityToChar: Record<string, string> = {}
for (const [char, entity] of Object.entries(charToEntity)) {
  ...
  try {
    ...
    for (const char of validated.value) {
      if (options.mode === 'minimal') {
        ...
        // "all" mode: use named entity if available, otherwise use numeric entity for non-ASCII
...
export function decodeHtmlEntities(input: string): Result<string> {
  ...
  try {
    const result = validated.value.replace(/&(#x[0-9a-fA-F]+|#X[0-9a-fA-F]+|#[0-9]+|[a-zA-Z][a-zA-Z0-9]*);/g, (match) => {
      ...
      // Unrecognized entity, leave as-is
      return match
    })
    return ok(result)
  } catch {
    return err('DECODE_ERROR', 'Error: unable to decode the text')
  }
}
