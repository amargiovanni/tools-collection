...
New approach:
- **QR Generation**: client-side via `qrcode` npm package by soldair (lightweight, no external API dependency, works offline)
...
- If a hash matching a known tool ID is found, redirect to the new path:
  ```ts
  // Maps old hash IDs to new tool slugs (renamed tools only need explicit entry)
  ...
  if (hash) {
    const slug = hashToSlug[hash] ?? hash  // renamed or same ID
    const lang = localStorage.getItem('lang') ?? navigator.language.slice(0, 2) === 'it' ? 'it' : 'en'
    window.location.replace(`/${lang}/tools/${slug}`)
  }
  ```
- Redirect uses `window.location.replace()` (no history entry)
- Language detected from localStorage (persisted choice) → browser language → fallback `en`
...
export default defineConfig({
  ...
  integrations: [
    solidJs(),
    paraglide({ project: './project.inlang', outdir: './src/i18n' }),
  ],
  vite: {
    plugins: [tailwindcss()],
  },
...
- **Solid.js**: `@astrojs/solid-js` for `.tsx` island support
- **Tailwind 4**: via `@tailwindcss/vite` Vite plugin (not the legacy `@astrojs/tailwind` integration)
- **Paraglide**: `@inlang/paraglide-astro` for compile-time i18n
- **Cloudflare**: `@astrojs/cloudflare` adapter, `output: 'static'` for SSG
...
Zero `any`. Zero `@ts-ignore` / `@ts-expect-error`. Zero `as` casts unless at a third-party library type boundary where `exactOptionalPropertyTypes` causes friction (Solid.js, Astro). In that case, an `as` cast is permitted with a comment referencing the upstream type that requires it.
